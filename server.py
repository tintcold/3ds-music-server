...
class MusicProxyHandler(http.server.BaseHTTPRequestHandler):
    # Force HTTP/1.0 so the 3DS client doesn't need to handle chunked encoding.
    # Client simply reads until the connection closes.
    ...
    def _handle_stream(self, params):
        ...
        # Use absolute path for ffmpeg.exe in the current directory
        script_dir = os.path.dirname(os.path.abspath(__file__))
        ffmpeg_exe = os.path.join(script_dir, "ffmpeg.exe")
        
        # If not in local dir, hope it's on PATH
        if not os.path.exists(ffmpeg_exe):
            ffmpeg_exe = "ffmpeg"

        # Piped pipeline: yt-dlp (download) -> ffmpeg (transcode to OGG) -> proxy
        cmd_ytdlp = [
        ...
        cmd_ffmpeg = [
            ...
            "-ar", "44100",          # sample rate the 3DS will use
            "-ac", "2",              # stereo
            "-q:a", "4",             # quality (~128 kbps)
            "-f", "ogg",             # container
        ...
        try:
            # We redirect stderr to DEVNULL to ensure no text warnings corrupt the binary pipes
            proc_ytdlp = subprocess.Popen(
                cmd_ytdlp, 
            ...
            proc_ffmpeg = subprocess.Popen(
                cmd_ffmpeg,
                stdin=proc_ytdlp.stdout,
                stdout=subprocess.PIPE,
                stderr=subprocess.DEVNULL
            )
            
            # Allow proc_ytdlp to receive a SIGPIPE if proc_ffmpeg exits
            ...
            # Stream the stdout of ffmpeg to the client
            while True:
                chunk = proc_ffmpeg.stdout.read(8192)
                if not chunk:
                    break
